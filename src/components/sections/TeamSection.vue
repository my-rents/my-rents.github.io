...
const maxIndex = computed(() => Math.max(teamMembers.length - cardsPerView.value, 0))

watch(cardsPerView, (value) => {
  currentIndex.value = Math.min(currentIndex.value, Math.max(teamMembers.length - value, 0))
})
